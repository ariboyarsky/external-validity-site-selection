...
zero at every $c_s$. A constant shift at an interior candidate is
...
does not depend on its right-hand side once it is feasible, so on
$\mathcal Y_k$ both values are finite or both programs are unbounded for
every $y$. When they are finite, $v_-$ is a piecewise linear convex
...
can still be computed exactly on a grid of $y$ values or by enumerating
branches.
