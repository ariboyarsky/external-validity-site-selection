...
unbounded-in-the-null-space behavior of Proposition~\ref{prop:dichotomy} with a
finite LP whose width we can compute and optimize.
...
\Delta b_k)$. The formula \eqref{eq:envelope} coincides with Equation~(1.3) of
Boyarsky et al.\ (2026), evaluated at the site-augmentation direction. It gives a
...
LOO diagnostic of Boyarsky et al.\ (2026, \S4.4), now read in reverse:
...
\item \textbf{(A) Coverage-expanding candidate.} If $(C_m^{(k)}, C_r^{(k)})
  ...
  the unrestricted limit this regime \emph{collapses} infinite widths to finite
  ones.
...
\item \textbf{(B) Location-improving candidate.} If $(C_m^{(k)}, C_r^{(k)}) \in
  ...
  subspace. The binding vertex of the LP can shift, yielding a first-order width
  change of either sign. This is the regime in which a new site \emph{improves
  overlap} at the target without adding a new identification direction.

\item \textbf{(C) Feasibility-restoring (or -breaking) candidate.} If
  $\{\theta : \Sigma_\mathcal{S}\theta = -b_\mathcal{S}\}\cap \mathcal{R} =
  \emptyset$ --- an infeasibility signaled by the Farkas alternative of Boyarsky
  et al.\ (2026, \S4.4 (c)) --- a candidate $k$ can restore feasibility if its
  site-update direction pushes the affine subspace back into $\mathcal{R}$.
...
the unrestricted limit, and is a useful first screen that should be refined by
\eqref{eq:surrogate} when the restriction set is binding.
...
In our simulation (Section~\ref{sec:simulation}) we adopt (1) with $B$ calibrated
from the existing-pool residual variance; the three variants agree on the
minimax-optimal candidate in all our runs, which is unsurprising given that the
rank update in \eqref{eq:dsigma} dominates the first-order width change in the
regimes where candidates are materially distinguishable.
