...
\paragraph{Proposition 1 of Boyarsky et al.\ (2026).}
...
sizes and individual covariate overlap. When $S$ is small relative to
$d_M + d_I$, this matrix --- and hence $\Sigma$ --- is rank-deficient; this is
precisely why the \citet{meager2019understanding} micro-credit application with
$S=7$ sites exhibits non-trivial partial identification.

\paragraph{Partial-identification bounds.}
Boyarsky et al.\ (2026) define
\begin{equation}
\label{eq:bounds}
...
Throughout this paper we assume $r$ defines a compact polyhedron
$\mathcal{R} := \{\theta : r(\theta)\le 0\}$; this is the regime where the width
is finite and the LP/MIP program is well-posed. The extension to relaxed
