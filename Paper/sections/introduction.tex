...
infeasible. We
give a two-site example where one conflicting addition doubles the
width. The effect of the conflict on correctness is more important than its
effect on the width. The translated set stops covering the true effect
exactly at the targets the pool point identifies. The screens we propose, a rank
test on profiles and a compatibility check on outcomes, separate the
additions that refine the set from the additions that corrupt it.
