...
estimator, the predictions matched the realized experiments, and the
rule chose the better of two candidates in every one of thirty choices.
...
the screened pool. Choosing several sites at once is harder. The rank added by a set of
candidates is a matroid rank function, so greedy selection maximizes the
added rank exactly under a limit on the number of sites. The rank is not
the target-relevant objective, however. Whether a target enters the
augmented hull can require two candidates together. For example, a
target offset along $e_1+e_2$ needs a candidate along $e_1$ and one along
$e_2$, and neither helps alone. That objective is not submodular, and the
width objective can behave the same way, so a joint selection should
evaluate the width map at candidate sets rather than add sites one at a
time.
