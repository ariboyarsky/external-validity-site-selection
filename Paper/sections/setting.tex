...
moments used below are finite. The target covariate law overlaps the
source, in the sense that $\tP$ is absolutely continuous with respect to
the source covariate law $\sP_X$.
\end{assumption}
...
$r<p$, which always holds when $p>S-1$, the normal equations leave
fixed-effect directions undetermined, and $\psi_T$ is point identified
if and only if $\ellvec\in\range(\CovC)$.
...
$\range(\CovC)=\Vspan$ and $\kernel(\CovC)=\Vspan^\perp$. The second is the estimator of Algorithm~1 in \citet{boyarsky2026}. This algorithm first estimates the five inputs
...
coordinates. We call it the rank consistent estimator, as in \citet{boyarsky2026}, where it is contrasted with a one-step
estimator. Every estimate reported in this paper uses it.
